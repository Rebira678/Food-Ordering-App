import 'package:flutter/material.dart';

class AppColors {
  // Brand
  static const primary = Color(0xFF047857); // Darker Emerald Green
  static const success = Color(0xFF10B981);

  // Light theme
  static const lightBackground = Color(0xFFF7F8FA);
  static const lightCard = Color(0xFFFFFFFF);
  static const lightText = Color(0xFF1E222B);
  static const lightIcon = Color(0xFF687076);

  // Dark theme
  static const darkBackground = Color(0xFF111613);
  static const darkCard = Color(0xFF181E19);
  static const darkText = Color(0xFFECEDEE);
  static const darkIcon = Color(0xFF9BA1A6);

  // Misc
  static const errorRed = Color(0xFFEF4444);
  static const tipOrange = Color(0xFFFFE8D6);
  static const deliveryGreen = Color(0xFF00A86B);
  static const white = Color(0xFFFFFFFF);
}
